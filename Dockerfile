...
ADD . /build/
WORKDIR /build
RUN go build -o /build/locg-service -ldflags "-s -w -X locgame-mini-server/internal/version.RELEASE=${CI_COMMIT_BRANCH} -X locgame-mini-server/internal/version.COMMIT=${CI_COMMIT_SHA} -X locgame-mini-server/internal/version.REPO=${CI_PROJECT_URL} -X locgame-mini-server/internal/version.BUILD=${BUILD_NUMBER}" cmd/locgame-mini-server/service.go
...
RUN addgroup -g 1000 locg && adduser -u 1000 -G locg -s /bin/sh -D locg && \
    ...
    rm -rf /var/cache/apk/*
COPY --chown=locg:locg --from=builder /build/locg-service /app/
